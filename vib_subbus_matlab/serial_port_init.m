...
if (nargin < 1 || isempty(port)) && exist('./VIBSerialPort.mat','file')
  rport = load('./VIBSerialPort.mat');
...
if isempty(port)
  if isempty(hw.AvailableSerialPorts)
    % closereq;
    h = errordlg('No serial port found','VIB Serial Port Error','modal');
    uiwait(h);
    return;
  else
    sel = listdlg('ListString',hw.AvailableSerialPorts,...
      'SelectionMode','single','Name','VIB_Port', ...
